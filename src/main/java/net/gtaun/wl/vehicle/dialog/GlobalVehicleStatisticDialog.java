...
public class GlobalVehicleStatisticDialog
{
	public static WlMsgboxDialog create(Player player, EventManager eventManager, AbstractDialog parent, Vehicle vehicle, VehicleManagerServiceImpl service)
	{
		...
		return WlMsgboxDialog.create(player, eventManager)
			.parentDialog(parent)
			.caption((d) -> stringSet.format("Dialog.GlobalVehicleStatisticDialog.Caption", name, modelId))
			.message((d) ->
			{
				GlobalVehicleStatistic stat = service.getGlobalVehicleStatistic(modelId);
				
				String textFormat = stringSet.get("Dialog.GlobalVehicleStatisticDialog.Text");
